...
export default async function VectorUnitDetailPage({
  params,
}: {
  params: Promise<{ lang: string; vectorSlug: string; moduleType: string; unitSlug: string }>;
}) {
  const { lang, vectorSlug, moduleType, unitSlug } = await params;
      ...
      >
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-4 flex-wrap">
            <Link href="/language-hub" className="hover:text-[#8B5CF6] transition-colors">
              语言中枢
            </Link>
            <span>/</span>
            <Link href={`/language-hub/${lang}`} className="hover:text-[#8B5CF6] transition-colors">
              {lang === "python" ? "Python+" : lang === "java" ? "Java+" : "TypeScript+"}
            </Link>
            <span>/</span>
            <Link
            ...
            <span>/</span>
            <Link
              href={`/language-hub/${lang}/${vectorSlug}/module/${moduleType}`}
              className="hover:text-[#8B5CF6] transition-colors"
            >
              {meta.label}
            </Link>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <Badge className="border-0 text-white" style={{ backgroundColor: meta.color }}>
              {meta.label}
            </Badge>
            ...
            <span className="text-slate-500">{vector.name}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
            ...
            <div className="sticky top-24 rounded-xl border border-[#1E293B] bg-[#131A2B] overflow-hidden">
              ...
              <div className="max-h-[60vh] overflow-y-auto">
                {moduleUnits.map((vu: any, i: number) => {
                  const isActive = vu.unit.slug === unitSlug;
                  return (
                    <Link
                      key={vu.id}
          ...
          <main className="lg:col-span-3 order-1 lg:order-2 min-w-0">
            ...
            <div className="flex items-stretch gap-4">
              {prevUnit ? (
                <Link
                  href={`/language-hub/${lang}/${vectorSlug}/module/${moduleType}/${prevUnit.slug}`}
                  className="flex-1 group rounded-xl border border-[#1E293B] bg-[#131A2B] p-4 hover:border-[#8B5CF6] transition-all"
                >
              ...
              {nextUnit ? (
                <Link
                  href={`/language-hub/${lang}/${vectorSlug}/module/${moduleType}/${nextUnit.slug}`}
                  className="flex-1 group rounded-xl border border-[#1E293B] bg-[#131A2B] p-4 hover:border-[#8B5CF6] transition-all text-right"
                >
